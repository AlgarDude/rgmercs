    ...
    if ImGui.BeginTable("UserModulesTable", 8, bit32.bor(ImGuiTableFlags.Borders, ImGuiTableFlags.Resizable)) then
        ...
        for _, row in ipairs(rows) do
            ...
            if isLoaded and frameTime then
                Ui.RenderColoredText(frameTime < 1 and Globals.Constants.Colors.ConditionPassColor or
                    (frameTime < 5 and Globals.Constants.Colors.ConditionMidColor or Globals.Constants.Colors.ConditionFailColor),
                    "%.2f", frameTime)
                Ui.Tooltip("Average time this module spends in the RGMercs loop each pass.")
            else
